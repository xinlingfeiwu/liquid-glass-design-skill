...
4. Apply the material selectively. Prefer bars, floating controls, popovers, pills, cards, and selection indicators. Do not blanket the content layer in glass.
5. Verify against the checklist before calling the work complete.
...
- Use shadows, inner rims, pointer-aware glare, and small specular glints as a system. Avoid large white sweeps, muddy blur, or one-note transparency.
...
- Surface fill: a glossy top-sheen gradient over `rgba(32, 42, 56, .20)`; raise tint opacity only for bright/noisy content that defeats legibility.
- Border: 1px `rgba(255, 255, 255, .52)` plus a fine inner rim and a bright top edge (pseudo-element).
- Refractive path (Chromium/Electron): `backdrop-filter: url(#per-surface-filter) blur(0.2px) saturate(1.55) brightness(1.1) contrast(1.14)`.
- Cross-browser fallback (Safari/Firefox): same color ops with `blur(6px)` and no filter URL.
- Lens tuning: `magnify 1`, `strength 100%` of the measured scale, `bezelRatio 0.62`, `spread 0.58`, `dispersion 0.07`.
...
- The demo/verification backdrop is bright and detailed enough to actually show refraction.
